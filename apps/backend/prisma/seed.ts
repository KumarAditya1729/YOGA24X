...
async function main() {
  console.log(
    "🌱 Starting Yoga24X Enterprise Database Seed (v2.0 Identity & User Management)...",
  );
  ...
  const rolesData = [
    {
      name: "SUPER_ADMIN",
      description:
        "System owner with unrestricted access across all tenants and modules",
    },
    {
      name: "ADMIN",
      description:
        "System administrator with user management, moderation, and support access",
    },
    {
      name: "STUDENT",
      description:
        "Standard learner accessing AI coaching, live classes, courses, and bookings",
    },
    {
      name: "TEACHER",
      description:
        "Verified instructor teaching live classes, publishing courses, and mentoring",
    },
    {
      name: "DOCTOR",
      description:
        "Ayurvedic & medical practitioner providing consultations and clinical reviews",
    },
    {
      name: "NUTRITIONIST",
      description:
        "Dietary specialist creating personalized Sattvic and Ayurvedic meal plans",
    },
    {
      name: "MEDITATION_COACH",
      description:
        "Mindfulness guide leading guided pranayama and meditation retreats",
    },
    {
      name: "STUDIO_OWNER",
      description:
        "Partner managing physical yoga studios, teacher rosters, and hybrid classes",
    },
    {
      name: "CORPORATE_HR",
      description:
        "Enterprise wellness coordinator monitoring corporate employee engagement KPIs",
    },
  ];
  ...
  const permissionsData = [
    {
      module: "auth",
      action: "login",
      description: "Perform standard authentication",
    },
    {
      module: "auth",
      action: "refresh",
    ...
    },
    {
      module: "auth",
      action: "logout",
      description: "Terminate active user session",
    },
    {
      module: "iam",
      action: "manage_users",
      description: "Create, edit, suspend, or delete user accounts",
    },
    {
      module: "iam",
      action: "view_sessions",
      description: "View active sessions across all users",
    },
    {
      module: "iam",
      action: "revoke_sessions",
      description: "Force logout users and terminate sessions",
    },
    {
      module: "iam",
      action: "unlock_accounts",
      description: "Unlock rate-limited or brute-force locked accounts",
    },
    {
      module: "iam",
      action: "manage_devices",
      description: "View and revoke trusted biometric devices",
    },
    {
      module: "iam",
      action: "manage_organizations",
      description: "Create and manage studios and corporate accounts",
    },
    {
      module: "iam",
      action: "verify_identities",
      description: "Review and approve verification documents",
    },
    {
      module: "lms",
      action: "create_course",
      description: "Publish and manage video learning courses",
    },
    {
      module: "booking",
      action: "book_slot",
      description: "Book live yoga classes and doctor consultations",
    },
    {
      module: "ai_coach",
      action: "start_session",
      description: "Initiate 30fps edge AI pose evaluation session",
    },
  ];
  ...
  for (const p of permissionsData) {
    const perm = await prisma.permission.upsert({
      where: {
        uq_permissions_module_action: { module: p.module, action: p.action },
      },
  ...
  for (const u of usersToSeed) {
    ...
    await prisma.userIdentity.upsert({
      where: {
        uq_identities_provider_id: { provider: "EMAIL", providerId: u.email },
      },
      update: {},
    ...
    await prisma.userIdentity.upsert({
      where: {
        uq_identities_provider_id: { provider: "PHONE", providerId: u.phone },
      },
      update: {},
    ...
    if (u.role === "SUPER_ADMIN") {
      await prisma.superAdminProfile.upsert({
        ...
        create: {
          userId: user.id,
          securityClearanceLevel: "LEVEL_5",
      ...
      await prisma.adminProfile.upsert({
        ...
        create: {
          ...
          department: "IAM_OPERATIONS",
          assignedRegions: ["APAC", "EMEA"],
        },
      });
    ...
    console.log(
      `  ✅ User & Profile seeded: ${u.firstName} ${u.lastName} (${u.role})`,
    );
  }
  ...
  await prisma.featureFlag.upsert({
    ...
    create: {
      key: "ai_pose_coach_v2_edge",
      description: "30fps Edge AI Pose Evaluation",
      isEnabled: true,
  ...
  await prisma.featureFlag.upsert({
    ...
    create: {
      ...
      description: "WebAuthn / Passkey Biometric Login",
      isEnabled: true,
      rolloutPercentage: 100,
    },
  });
  ...
  if (studentId) {
    await prisma.healthProfile.upsert({
      ...
      create: {
        ...
        medicalHistoryJson: [
          {
            condition: "Mild Lower Back Stiffness",
            diagnosedYear: 2024,
            status: "MANAGED",
          },
        ],
        ...
        medicationsJson: [],
        allergiesJson: [
          { allergen: "Peanuts", type: "FOOD", severity: "MILD" },
        ],
        physicalLimitationsJson: [
          {
            bodyPart: "Lower Back",
            issue: "Stiffness",
            severity: "MEDIUM",
            restrictedMovements: ["Deep Backward Bends"],
          },
        ],
        lifestyleJson: {
          isSmoker: false,
        ...
        verificationNotes:
          "Reviewed lumbar stiffness. Recommended gentle forward folds and avoiding forced backbends.",
      },
    });
    ...
    await prisma.userGoal.create({
      data: {
        ...
        description:
          "Achieve comfortable full forward fold (Uttanasana) without lumbar strain.",
        targetValue: 100,
        currentValue: 65,
        unit: "PERCENTAGE",
        status: "ACTIVE",
        ...
        milestonesJson: [
          {
            title: "Hold Ardha Uttanasana for 60 seconds with straight spine",
            isCompleted: true,
          },
          {
            title: "Touch fingertips to floor without bending knees",
            isCompleted: false,
          },
    ...
    await prisma.medicalSafetyFlag.create({
      data: {
        ...
        description:
          "Avoid forced backbends or sudden spinal hyperextension due to lumbar stiffness.",
    ...
    console.log(
      "  ✅ Enterprise Wellness Profiles, Assessments, Goals & Safety Flags seeded.",
    );
  }
