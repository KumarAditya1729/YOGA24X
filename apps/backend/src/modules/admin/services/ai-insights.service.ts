...
export class AiInsightsService {
  ...
  async generateChurnPrediction(tenantId: string) {
    ...
    const provider = await this.providerRegistry.getBestAvailableProvider(AiModelRole.REASONING);
    if (!provider) {
    ...
    const res = await provider.chat({
      messages: [
        { role: 'system', content: 'You are an AI Business Analyst. Return JSON.' },
        { role: 'user', content: 'Analyze this sample engagement data and predict churn.' }
      ]
    });
    return res;
  }
