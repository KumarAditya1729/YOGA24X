...
class StubProvider implements AiProvider {
  ...
  getProviderType() { return this.type; }
  getPriority() { return this.priority; }
  async isAvailable() { return this.available; }
  getSupportedRoles() { return this.roles; }
...
describe('AiProviderRegistry', () => {
  ...
  it('should sort providers by priority on registration', () => {
    ...
    registry.registerProvider(new StubProvider(10, true, AiProviderType.OPENAI));
    registry.registerProvider(new StubProvider(50, true, AiProviderType.OLLAMA));
  ...
  it('should return the best available provider for a role', async () => {
    ...
    registry.registerProvider(new StubProvider(10, false, AiProviderType.OPENAI));
  ...
  it('should fallback to mock if all higher priority providers are unavailable', async () => {
    registry.registerProvider(new StubProvider(10, false, AiProviderType.OPENAI));
    registry.registerProvider(new StubProvider(50, false, AiProviderType.OLLAMA));
    registry.registerProvider(new StubProvider(100, true, AiProviderType.MOCK));
  ...
  it('should throw an error if no provider is available', async () => {
    ...
    await expect(registry.getBestAvailableProvider(AiModelRole.COACH))
  ...
  it('should skip providers that do not support the requested role', async () => {
    ...
    registry.registerProvider(new StubProvider(10, true, AiProviderType.OPENAI, [AiModelRole.EMBEDDING]));
    
    // Lower priority, available, supports COACH
    registry.registerProvider(new StubProvider(50, true, AiProviderType.OLLAMA, [AiModelRole.COACH]));
