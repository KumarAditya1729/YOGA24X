...
export class AiProviderRegistry implements OnModuleInit {
  ...
  registerProvider(provider: AiProvider) {
    ...
    this.logger.log(`Registered AI Provider: ${provider.getProviderType()} (Priority: ${provider.getPriority()})`);
   ...
   */
  async getBestAvailableProvider(role: AiModelRole = AiModelRole.CHAT): Promise<AiProvider> {
    for (const provider of this.providers) {
      if (provider.getSupportedRoles().includes(role)) {
        try {
          const isAvailable = await provider.isAvailable();
          if (isAvailable) {
            this.logger.debug(`Selected provider ${provider.getProviderType()} for role ${role}`);
            return provider;
          }
        } catch (error) {
          this.logger.warn(`Provider ${provider.getProviderType()} is unavailable: ${error.message}`);
        }
      }
    }
    
    throw new Error(`No available AI providers for role: ${role}. System requires at least a Mock or Rule-based fallback provider.`);
  }
}
