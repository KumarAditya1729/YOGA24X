...
export class CoachService {
  ...
  async chatWithCoach(userId: string, conversationId: string, message: string) {
    ...
    const sanitizedMessage = await this.safetyService.validateAndSanitize(
      userId,
      message,
    );
    ...
    const systemPrompt = await this.promptManager.getCoachSystemPrompt(
    ...
    // 6. Execute Chat
    this.logger.debug(
      `Executing chat via ${provider.getProviderType()} for conversation ${conversationId}`,
    );
