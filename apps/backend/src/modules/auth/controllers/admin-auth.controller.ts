...
import { Controller, Get, Post, Delete, Param, Body, Query, UseGuards, HttpCode, HttpStatus, NotFoundException } from '@nestjs/common';
...
export class AdminAuthController {
  ...
  async getUserLoginHistory(@Param('userId') userId: string, @Query('limit') limitStr = '30') {
    const limit = parseInt(limitStr, 10) || 30;
    const history = await this.authRepository.listRecentLoginHistory(userId, limit);
  ...
  async forceLogoutUser(@Param('userId') userId: string) {
    ...
    return { success: true, message: `Terminated ${count} active session(s) for user ${userId}` };
  }
  ...
  @HttpCode(HttpStatus.OK)
  async updateUserStatus(@Param('userId') userId: string, @Body('status') status: 'ACTIVE' | 'SUSPENDED' | 'BANNED' | 'DELETED') {
    const user = await this.authRepository.updateUserStatus(userId, status as any);
    if (status !== 'ACTIVE') {
      await this.sessionService.revokeAllUserSessions(userId);
    }
    return { success: true, message: `User status updated to ${status}`, data: user };
  }
  ...
  async unlockAccount(@Param('userId') userId: string) {
    ...
    const lockKeyPhone = user.phoneNumber ? `${AUTH_CONSTANTS.REDIS_KEY_ACCOUNT_LOCK_PREFIX}${user.phoneNumber}` : null;
    ...
    return { success: true, message: `Account unlocked successfully for ${user.email}` };
  }
}
