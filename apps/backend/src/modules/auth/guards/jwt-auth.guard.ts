...
export class JwtAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      const payload = await this.jwtService.verifyAsync<JwtAccessPayload>(token, {
        secret: process.env.JWT_ACCESS_SECRET || 'yoga24x-enterprise-access-secret-key-2026',
      });
      ...
      if (isLocked) {
        throw new ForbiddenException('Account temporarily locked due to security alerts');
      }
  ...
  private extractTokenFromHeaderOrCookie(request: any): string | undefined {
    ...
    if (request.cookies && request.cookies[AUTH_CONSTANTS.COOKIE_ACCESS_TOKEN]) {
