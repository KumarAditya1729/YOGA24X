...
export const RateLimit = (limit = 20, windowSeconds = 60) => (target: any, key?: string | symbol, descriptor?: PropertyDescriptor) => {
  Reflector.createDecorator<({ limit: number; windowSeconds: number })>()({ limit, windowSeconds })(target, key as any, descriptor as any);
};
...
export class RateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const config = this.reflector.getAllAndOverride<{ limit: number; windowSeconds: number }>(RATE_LIMIT_KEY, [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    const res = await this.redisService.checkRateLimit(key, config.limit, config.windowSeconds);
