...
export class AuthService {
  ...
  async loginWithPassword(
    ...
    if (!rateLimit.allowed) {
      throw new ForbiddenException(
        `Too many login attempts. Please wait ${rateLimit.resetInSeconds} seconds.`,
      );
    }
    ...
    if (!user || !user.passwordHash) {
      await this.riskService.logLoginAttempt(
    ...
    if (user.passwordHash.includes(":")) {
      ...
      const verifyHash = crypto
        .pbkdf2Sync(dto.password, salt, 100000, 64, "sha512")
        .toString("hex");
    ...
    const risk = await this.riskService.evaluateLoginRisk(
      user.id,
      dto.deviceInfo,
      ipAddress,
    );
    if (risk.blockLogin) {
      await this.riskService.logLoginAttempt(
        ...
        dto.deviceInfo.deviceType,
        risk.riskScore,
        "Critical risk score blocked",
      );
      throw new ForbiddenException(
        "Login blocked due to suspicious activity. Please verify your identity via email recovery.",
      );
    }
    ...
    if (user.twoFactorEnabled || risk.requireMfa) {
      ...
      throw new ForbiddenException({
        ...
        error: "MFA_REQUIRED",
        message:
          "Two-factor authentication required. An OTP has been dispatched to your verified email.",
        userId: user.id,
    ...
    return this.completeLoginOrchestration(
      user,
      dto.deviceInfo,
      ipAddress,
      userAgent,
  ...
  async loginWithBiometric(
    ...
    if (
      ...
      trustedDevice.userId !== dto.userId
    ) {
      throw new UnauthorizedException(
        "Biometric passkey device is not registered or revoked",
      );
    }
    ...
    const expectedChallenge = crypto
      .createHash("sha256")
      .update(`${dto.userId}:${dto.deviceFingerprint}`)
      .digest("hex");
    if (!dto.cryptographicSignature || dto.cryptographicSignature.length < 20) {
      throw new UnauthorizedException(
        "Invalid biometric passkey cryptographic signature",
      );
    }
