...
export class OAuthService {
  ...
  constructor(private readonly authRepository: AuthRepository) {
    this.googleClient = new OAuth2Client(
      process.env.GOOGLE_CLIENT_ID || 'sample-google-client-id.apps.googleusercontent.com',
    );
  ...
  async verifyGoogleIdToken(idToken: string, defaultRole: UserRoleName = 'STUDENT'): Promise<{
    user: any;
    isNewUser: boolean;
  }> {
    try {
      const ticket = await this.googleClient.verifyIdToken({
        idToken,
        audience: process.env.GOOGLE_CLIENT_ID || 'sample-google-client-id.apps.googleusercontent.com',
      });
      ...
      let user = await this.authRepository.findUserByIdentity('GOOGLE', googleId);
      let isNewUser = false;
  ...
  private async getAppleSigningKey(kid: string): Promise<jwksClient.SigningKey> {
