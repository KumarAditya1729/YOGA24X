...
import { AUTH_CONSTANTS, DeviceInfo, RiskEvaluationResult } from '@yoga24x/shared-types';
...
export class RiskService {
  ...
  async evaluateLoginRisk(
    ...
    if (blockedIp) {
      throw new ForbiddenException(`Access denied from blocked IP address: ${ipAddress}. Reason: ${blockedIp.reason}`);
    }

    // 2. FACTOR 1: Is this a completely new device fingerprint for this user? (+30 risk)
    const isTrusted = await this.authRepository.findTrustedDeviceByFingerprint(deviceInfo.deviceFingerprint);
    ...
    const recentLogins = await this.authRepository.listRecentLoginHistory(userId, 5);
    if (recentLogins.length > 0) {
      ...
      if (lastLogin.location && lastLogin.location !== location && location !== 'Unknown') {
        riskScore += 25;
    ...
    await this.redisService.set(evalKey, JSON.stringify({ riskScore, riskLevel, factors, ipAddress }), 3600);
  ...
  async logLoginAttempt(
    ...
    if (status === 'FAILED' || status === 'BLOCKED' || status === 'SUSPICIOUS') {
      const failedLoginsKey = `${AUTH_CONSTANTS.REDIS_KEY_FAILED_LOGINS_PREFIX}${ipAddress}`;
      ...
      await this.authRepository.createAuditLog({
        actorId: userId,
        action: status === 'SUSPICIOUS' ? AUTH_CONSTANTS.AUDIT_ACTION_SUSPICIOUS_LOGIN_DETECTED : AUTH_CONSTANTS.AUDIT_ACTION_LOGIN_FAILED,
