...
export class PaymentController {
  ...
  async razorpayWebhook(
    ...
    return this.paymentService.handleWebhook('razorpay', eventId, payload?.event ?? 'unknown', payload, signature);
  }
  ...
  async stripeWebhook(
    ...
    return this.paymentService.handleWebhook('stripe', payload?.id ?? `evt_${Date.now()}`, payload?.type ?? 'unknown', payload, signature);
  }
