...
import {
  ...
  PurchaseGiftCardDto, RedeemGiftCardDto,
...
export class PricingController {
  ...
  listCoupons() { return this.pricingService.listCoupons(); }

  @Post('coupons')
  @RequirePermissions(PERMISSIONS.COUPON_MANAGE)
  @ApiOperation({ summary: 'Create a coupon (Admin)' })
  createCoupon(@Body() dto: CreateCouponDto) { return this.pricingService.createCoupon(dto); }
  ...
  getRules() { return this.pricingService.getActivePricingRules(); }

  @Post('rules')
  @RequirePermissions(PERMISSIONS.PRICING_MANAGE)
  @ApiOperation({ summary: 'Create a pricing rule (Admin)' })
  createRule(@Body() dto: CreatePricingRuleDto) { return this.pricingService.createPricingRule(dto); }
  ...
  calculatePrice(
    ...
    return this.pricingService.calculatePrice(body.basePriceCents, req.user.userId, body.couponCode);
  }
}
