...
describe('PricingRepository', () => {
  ...
  describe('validateAndApplyCoupon', () => {
    it('should throw NotFoundException if coupon does not exist', async () => {
      ...
      await expect(
        repository.validateAndApplyCoupon('user-1', { code: 'INVALID', orderAmountCents: 1000 })
    ...
    it('should calculate percentage discount correctly', async () => {
      ...
      const result = await repository.validateAndApplyCoupon('user-1', { code: 'SUMMER20', orderAmountCents: 10000 });
    ...
    it('should throw BadRequestException if per-user limit reached', async () => {
      ...
      await expect(
        repository.validateAndApplyCoupon('user-1', { code: 'ONCE', orderAmountCents: 10000 })
  ...
  describe('calculateFinalPrice', () => {
    it('should apply pricing rules', async () => {
      jest.spyOn(prisma.pricingRule, 'findMany').mockResolvedValue([
        { discountCents: 1000, ruleType: 'PROMO' } as any,
      ]);
