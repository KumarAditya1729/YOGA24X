...
describe("RefundRepository", () => {
  ...
  describe("requestRefund", () => {
    it("should throw NotFoundException if payment not found", async () => {
      jest
        .spyOn(prisma.paymentTransaction, "findUnique")
        .mockResolvedValue(null);

      await expect(
        repository.requestRefund("user-1", {
    ...
    it("should create a refund request", async () => {
      ...
      const result = await repository.requestRefund("user-1", {
        paymentTransactionId: "tx-1",
        amountCents: 1000,
        reason: RefundReason.OTHER,
      });
  ...
  describe("processRefund", () => {
    it("should emit event when approved", async () => {
      ...
      await repository.processRefund("admin-1", {
        refundId: "ref-1",
        decision: "APPROVED",
      });
