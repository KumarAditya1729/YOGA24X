...
export class RevenueRepository {
  ...
  async createRevenueShare(paymentTransactionId: string, dto: CreateRevenueShareRuleDto, amountCents: number) {
  ...
  async calculateAndDistributeRevenue(paymentTransactionId: string, grossAmountCents: number) {
    // Default platform commission: 20%, teacher: 70%, studio: 10%
    const platformShare = Math.floor(grossAmountCents * 0.20);
    const teacherShare = Math.floor(grossAmountCents * 0.70);
