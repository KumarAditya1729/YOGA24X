...
import { Controller, Get, Put, Post, Delete, Body, Param, Request } from '@nestjs/common';
...
export class DeviceController {
  ...
  async updateNickname(@Request() req: any, @Param('id') deviceId: string, @Body() body: { nickname: string }): Promise<any> {
    const userId = req.user?.sub || req.headers['x-user-id'];
    return this.deviceService.updateDeviceNickname(userId, deviceId, body.nickname);
  }

  @Delete('devices/:id')
  async removeDevice(@Request() req: any, @Param('id') deviceId: string): Promise<{ success: boolean }> {
    const userId = req.user?.sub || req.headers['x-user-id'];
  ...
  @Post('identities')
  async linkIdentity(@Request() req: any, @Body() body: { provider: string; providerId: string; profileDataJson?: Record<string, any> }): Promise<any> {
    const userId = req.user?.sub || req.headers['x-user-id'];
    return this.deviceService.linkIdentity(userId, body.provider, body.providerId, body.profileDataJson);
  }

  @Delete('identities/:provider/:providerId')
  async unlinkIdentity(@Request() req: any, @Param('provider') provider: string, @Param('providerId') providerId: string): Promise<{ success: boolean }> {
    const userId = req.user?.sub || req.headers['x-user-id'];
