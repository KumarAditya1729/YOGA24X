...
import { SubmitVerificationDto, ReviewVerificationDto } from '@yoga24x/shared-types';
...
export class VerificationController {
  ...
  @Post('submit')
  async submitVerification(@Request() req: any, @Body() dto: SubmitVerificationDto): Promise<any> {
  ...
  @Get('admin/queue')
  async getAdminQueue(@Query('status') status?: string, @Query('type') type?: string): Promise<any[]> {
    return this.verificationService.getAdminVerificationQueue(status, type);
  }

  @Put('admin/:id/review')
  async reviewVerification(@Request() req: any, @Param('id') id: string, @Body() dto: ReviewVerificationDto): Promise<any> {
