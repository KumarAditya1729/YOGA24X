...
export class PiiMaskingInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return next.handle().pipe(
      map((data) => {
        ...
        const isAdmin = viewer && (viewer.roles?.includes('SUPER_ADMIN') || viewer.roles?.includes('ADMIN'));
  ...
  private maskObject(obj: any): any {
    ...
    if (typeof masked.phoneNumber === 'string' && masked.phoneNumber.length > 4) {
      const len = masked.phoneNumber.length;
      masked.phoneNumber = masked.phoneNumber.substring(0, 3) + '*'.repeat(len - 7) + masked.phoneNumber.substring(len - 4);
    }

    // Mask emergency contact phone
    if (masked.studentProfile && typeof masked.studentProfile.emergencyContactPhone === 'string') {
      const len = masked.studentProfile.emergencyContactPhone.length;
      if (len > 4) {
        masked.studentProfile.emergencyContactPhone =
          masked.studentProfile.emergencyContactPhone.substring(0, 3) + '*'.repeat(len - 7) + masked.studentProfile.emergencyContactPhone.substring(len - 4);
      }
    }

    // Mask tax identification number in corporate profile
    if (masked.corporateProfile && typeof masked.corporateProfile.taxIdentificationNumber === 'string') {
      const len = masked.corporateProfile.taxIdentificationNumber.length;
      if (len > 4) {
        masked.corporateProfile.taxIdentificationNumber =
          masked.corporateProfile.taxIdentificationNumber.substring(0, 2) + '*'.repeat(len - 6) + masked.corporateProfile.taxIdentificationNumber.substring(len - 4);
      }
    }
