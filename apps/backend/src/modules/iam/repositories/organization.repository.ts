...
import { CreateOrganizationDto, InviteOrgMemberDto, IAM_CONSTANTS } from '@yoga24x/shared-types';
...
export class OrganizationRepository {
  ...
  async createOrganization(ownerId: string, dto: CreateOrganizationDto): Promise<any> {
  ...
  async getOrganizationDetails(orgId: string): Promise<any> {
    const org = await this.prisma.organization.findUnique({
      ...
      include: {
        ...
        teams: { include: { leadUser: { select: { id: true, firstName: true, lastName: true } } } },
        departments: true,
        branches: { include: { managerUser: { select: { id: true, firstName: true, lastName: true } } } },
        members: {
          include: {
  ...
  async inviteMember(orgId: string, inviterId: string, dto: InviteOrgMemberDto): Promise<any> {
    // Check if user is already a member
    const existingUser = await this.prisma.user.findUnique({ where: { email: dto.email } });
    if (existingUser) {
      const existingMember = await this.prisma.organizationMember.findUnique({
        where: { uq_org_member: { organizationId: orgId, userId: existingUser.id } },
      });
      if (existingMember) {
  ...
  async acceptInvitation(token: string, userId: string): Promise<any> {
    return this.prisma.$transaction(async (tx: any) => {
      ...
      if (!invite || invite.status !== 'PENDING' || invite.expiresAt < new Date()) {
        throw new ForbiddenException('Invitation is invalid, expired, or already used');
      }
  ...
  async removeMember(orgId: string, removerId: string, targetUserId: string): Promise<void> {
    const removerRole = await this.getMemberRole(orgId, removerId);
    if (removerRole !== 'OWNER' && removerRole !== 'ADMIN') {
      throw new ForbiddenException(IAM_CONSTANTS.ERROR_CODES.UNAUTHORIZED_ORG_ACCESS);
    }

    const targetRole = await this.getMemberRole(orgId, targetUserId);
    if (targetRole === 'OWNER') {
      throw new ConflictException(IAM_CONSTANTS.ERROR_CODES.CANNOT_DELETE_ONLY_OWNER);
    }
  ...
  async createTeam(orgId: string, name: string, description?: string, leadUserId?: string): Promise<any> {
    return this.prisma.organizationTeam.create({
      data: { organizationId: orgId, name, description, leadUserId },
    });
  }

  async createDepartment(orgId: string, name: string, budgetCode?: string): Promise<any> {
    return this.prisma.organizationDepartment.create({
      data: { organizationId: orgId, name, budgetCode },
    });
  }

  async createBranch(orgId: string, name: string, address?: string, city?: string, managerUserId?: string): Promise<any> {
