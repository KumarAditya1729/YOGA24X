...
import { SubmitVerificationDto, ReviewVerificationDto, IAM_CONSTANTS } from '@yoga24x/shared-types';

@Injectable()
export class VerificationRepository {
  constructor(private readonly prisma: PrismaService) {}

  async submitVerificationRequest(userId: string, dto: SubmitVerificationDto): Promise<any> {
    ...
    if (existing && existing.status === 'APPROVED') {
      throw new ConflictException(IAM_CONSTANTS.ERROR_CODES.VERIFICATION_ALREADY_APPROVED);
    }
  ...
  async reviewVerification(requestId: string, reviewerId: string, dto: ReviewVerificationDto): Promise<any> {
    ...
    return this.prisma.$transaction(async (tx: any) => {
      ...
      if (dto.status === 'APPROVED') {
        if (req.verificationType === 'EMAIL') {
          await tx.user.update({ where: { id: req.userId }, data: { isEmailVerified: true } });
        } else if (req.verificationType === 'PHONE') {
          await tx.user.update({ where: { id: req.userId }, data: { isPhoneVerified: true } });
        } else if (req.verificationType === 'TEACHER_CERT') {
          await tx.instructorProfile.updateMany({ where: { userId: req.userId }, data: { isVerified: true } });
        } else if (req.verificationType === 'DOCTOR_REG') {
          await tx.doctorProfile.updateMany({ where: { userId: req.userId }, data: { isVerified: true } });
        } else if (req.verificationType === 'STUDIO_REG') {
          await tx.studioProfile.updateMany({ where: { userId: req.userId }, data: { isVerified: true } });
        } else if (req.verificationType === 'CORPORATE_REG') {
          await tx.corporateProfile.updateMany({ where: { userId: req.userId }, data: { isVerified: true } });
        }
      }
