...
export class AccountLifecycleService {
  ...
  async suspendAccount(adminId: string, targetUserId: string, reason: string): Promise<any> {
    const user = await this.prisma.user.findUnique({ where: { id: targetUserId } });
    if (!user) throw new NotFoundException('User not found');

    const suspended = await this.privacyRepo.suspendAccount(targetUserId, reason);
  ...
  async searchUsers(query?: string, role?: string, status?: string): Promise<any[]> {
    const where: any = {};
    if (query) {
      where.OR = [
