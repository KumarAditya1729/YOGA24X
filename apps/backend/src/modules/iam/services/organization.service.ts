...
export class OrganizationService {
  ...
  async inviteMember(orgId: string, inviterId: string, dto: InviteOrgMemberDto): Promise<any> {
    ...
    if (role !== 'OWNER' && role !== 'ADMIN') {
      throw new ForbiddenException('Only organization owners and administrators can invite new members');
    }
  ...
  async createTeam(orgId: string, userId: string, name: string, description?: string, leadUserId?: string): Promise<any> {
    const role = await this.orgRepo.getMemberRole(orgId, userId);
    if (role !== 'OWNER' && role !== 'ADMIN') {
      throw new ForbiddenException('Only organization owners and administrators can create teams');
    }
    return this.orgRepo.createTeam(orgId, name, description, leadUserId);
  }

  async createDepartment(orgId: string, userId: string, name: string, budgetCode?: string): Promise<any> {
    const role = await this.orgRepo.getMemberRole(orgId, userId);
    if (role !== 'OWNER' && role !== 'ADMIN') {
      throw new ForbiddenException('Only organization owners and administrators can create departments');
    }
    return this.orgRepo.createDepartment(orgId, name, budgetCode);
  }

  async createBranch(orgId: string, userId: string, name: string, address?: string, city?: string, managerUserId?: string): Promise<any> {
    const role = await this.orgRepo.getMemberRole(orgId, userId);
    if (role !== 'OWNER' && role !== 'ADMIN') {
      throw new ForbiddenException('Only organization owners and administrators can create studio branches');
    }
