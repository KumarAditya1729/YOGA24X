...
export class ProfileService {
  ...
  async calculateProfileCompletion(userId: string): Promise<ProfileCompletionResult> {
    ...
    if (profile.roles.includes('STUDENT')) {
      if (profile.studentProfile?.gender && profile.studentProfile?.experienceLevel) {
        completedFields.push('studentProfile');
        score += 30;
      } else {
        missingRequiredFields.push('student dosha & experience level');
        nextRecommendedAction = 'Complete your Ayurvedic Dosha assessment and experience level';
      }
    } else if (profile.roles.includes('TEACHER')) {
      if (profile.instructorProfile?.bio && profile.instructorProfile?.specializations?.length > 0) {
        completedFields.push('instructorProfile');
        score += 30;
      } else {
        missingRequiredFields.push('teacher bio & specializations');
        nextRecommendedAction = 'Add your teaching bio, RYT certifications, and specializations';
      }
      ...
      if (profile.doctorProfile?.medicalRegistrationNumber) {
        ...
        nextRecommendedAction = 'Upload your Ayurvedic medical registration certificate for verification';
      }
    ...
    if (!nextRecommendedAction) {
      if (!profile.avatarUrl) nextRecommendedAction = 'Upload a professional profile photo';
      else if (!profile.isPhoneVerified) nextRecommendedAction = 'Verify your phone number with OTP';
  ...
  async uploadProfileMedia(
    ...
    if (!IAM_CONSTANTS.ALLOWED_IMAGE_MIMES.includes(mimeType as any)) {
      throw new BadRequestException(IAM_CONSTANTS.ERROR_CODES.INVALID_IMAGE_FORMAT);
    }

    const maxSize = mediaType === 'AVATAR' ? IAM_CONSTANTS.MAX_AVATAR_SIZE_BYTES : IAM_CONSTANTS.MAX_COVER_SIZE_BYTES;
    if (fileSizeBytes > maxSize) {
      throw new BadRequestException(`File size exceeds limit of ${maxSize / (1024 * 1024)}MB`);
    }

    const media = await this.profileRepo.addProfileMedia(userId, mediaType, fileUrl, fileSizeBytes, mimeType, fileUrl, fileUrl);
    await this.calculateProfileCompletion(userId);
    return media;
  }
}
