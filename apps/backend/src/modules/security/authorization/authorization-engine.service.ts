...
export class AuthorizationEngineService {
  ...
  private async doEvaluate(
    ...
    const overrideResult = await this.evaluateOverrides(ctx, requiredPermission);
    if (overrideResult !== null) {
      appliedOverrides.push(`override:${overrideResult.effect}`);
      if (overrideResult.effect === 'DENY') {
        return { permitted: false, decision: 'DENIED_PERMISSION', reason: `Explicit DENY override: ${overrideResult.reason}`, appliedOverrides, evaluatedPolicies };
      }
    ...
    if (!effectivePerms.has(requiredPermission)) {
      return { permitted: false, decision: 'DENIED_PERMISSION', reason: `Role '${ctx.roles.join(',')}' lacks permission '${requiredPermission}'`, appliedOverrides, evaluatedPolicies };
    }

    // ── Step 3: ABAC — evaluate active policies for this context ─────────────────
    const policyResult = await this.evaluatePolicies(ctx, requiredPermission, evaluatedPolicies);
    if (!policyResult.permitted) {
      return { permitted: false, decision: policyResult.decision, reason: policyResult.reason, appliedOverrides, evaluatedPolicies };
    }

    return { permitted: true, decision: 'PERMITTED', reason: 'RBAC + ABAC checks passed', appliedOverrides, evaluatedPolicies };
  }
  ...
  private async resolveEffectivePermissions(ctx: AuthorizationContext): Promise<Set<string>> {
    ...
    try {
      await this.redis.setex(cacheKey, this.PERM_CACHE_TTL, JSON.stringify([...perms]));
    } catch { /* non-critical */ }
  ...
  private async evaluateOverrides(
  ...
  ): Promise<{ effect: 'ALLOW' | 'DENY'; reason: string; isConditional: boolean } | null> {
    if (!ctx.userId) return null;

    const permission = await this.prisma.permission.findFirst({
      where: { module: permissionKey.split(':')[0], action: permissionKey.split(':')[1] },
    });
    ...
    if (override.state === 'TEMPORARY' && override.expiresAt && override.expiresAt < new Date()) {
  ...
  private async evaluatePolicies(
    ...
    let policies: Array<{ name: string; policyType: string; action: string; rulesJson: unknown; priority: number }>;
    ...
    try {
      ...
      if (cached) {
        ...
        policies = await this.prisma.securityPolicy.findMany({
          ...
          select: { name: true, policyType: true, action: true, rulesJson: true, priority: true },
        });
        await this.redis.setex(cacheKey, this.POLICY_CACHE_TTL, JSON.stringify(policies));
      }
    } catch {
      policies = [];
    }
      ...
      if (matches) {
        switch (policy.action) {
          ...
          case 'BLOCK_AND_ALERT':
            return { permitted: false, decision: `DENIED_POLICY`, reason: `Policy '${policy.name}' denied access` };
          case 'ENFORCE_MFA':
            if (!ctx.riskScore || ctx.riskScore < 50) break; // MFA already verified
            return { permitted: false, decision: 'MFA_REQUIRED', reason: `Policy '${policy.name}' requires MFA for this context` };
          case 'THROTTLE':
            // Throttling is handled by rate-limit guards; log but permit here
            this.logger.warn(`Throttle policy matched for user ${ctx.userId}: ${policy.name}`);
            break;
          case 'ALLOW':
          case 'AUDIT_ONLY':
            break; // Continue
    ...
    return { permitted: true, decision: 'PERMITTED', reason: 'No blocking policies matched' };
  }
  ...
  private matchesPolicyRules(
    ...
    if (rules['blockedCountries'] && Array.isArray(rules['blockedCountries'])) {
      if (ctx.countryCode && (rules['blockedCountries'] as string[]).includes(ctx.countryCode)) return true;
    }
    if (rules['allowedCountries'] && Array.isArray(rules['allowedCountries'])) {
      if (ctx.countryCode && !(rules['allowedCountries'] as string[]).includes(ctx.countryCode)) return true;
    }

    // Device trust rule
    if (rules['requireTrustedDevice'] === true && ctx.deviceTrust !== 'TRUSTED') return true;
    if (rules['blockUntrustedDevice'] === true && ctx.deviceTrust === 'BLOCKED') return true;

    // Time-based rule
    if (rules['allowedHours'] && Array.isArray(rules['allowedHours']) && ctx.currentHour !== undefined) {
      const [start, end] = rules['allowedHours'] as [number, number];
      if (ctx.currentHour < start || ctx.currentHour > end) return true;
    }

    // Risk score rule
    if (typeof rules['maxRiskScore'] === 'number' && ctx.riskScore !== undefined) {
      if (ctx.riskScore > (rules['maxRiskScore'] as number)) return true;
    }

    // Health data rule
    if (rules['requireHealthVerification'] === true && !ctx.hasHealthRestrictions) return false;

    // Subscription rule
    if (rules['requiredSubscriptionTiers'] && Array.isArray(rules['requiredSubscriptionTiers'])) {
      const tiers = rules['requiredSubscriptionTiers'] as string[];
      if (ctx.subscriptionTier && !tiers.includes(ctx.subscriptionTier)) return true;
    }
  ...
  private evaluateConditionExpression(
    ...
    if (condition['ipRange']) {
      ...
      if (ctx.ipAddress && !(ctx.ipAddress as string).startsWith(condition['ipRange'] as string)) {
        return false;
      }
    }
    ...
    if (condition['subscriptionTiers'] && Array.isArray(condition['subscriptionTiers'])) {
      if (!ctx.subscriptionTier || !(condition['subscriptionTiers'] as string[]).includes(ctx.subscriptionTier)) return false;
    }
