...
export class FeatureFlagsController {
  ...
  createOrUpdate(@Body() body: {
    ...
    return this.flagService.createOrUpdate({
      ...body,
      scheduledEnableAt: body.scheduledEnableAt ? new Date(body.scheduledEnableAt) : undefined,
      scheduledDisableAt: body.scheduledDisableAt ? new Date(body.scheduledDisableAt) : undefined,
    });
  }
  ...
  killSwitch(
    ...
    return this.flagService.triggerKillSwitch(key, body.tenantId, body.activatedBy);
  }
  ...
  @RequirePermissions(PERMISSIONS.FLAGS_KILL_SWITCH)
  @HttpCode(HttpStatus.NO_CONTENT)
  releaseKillSwitch(@Param('key') key: string, @Query('tenantId') tenantId?: string) {
...
export class PoliciesController {
  ...
  update(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() body: { name?: string; rulesJson?: Record<string, unknown>; isEnabled?: boolean; priority?: number },
  ) {
...
export class OrganizationsController {
  ...
  @HttpCode(HttpStatus.CREATED)
  create(@Body() body: { name: string; type: string; ownerId: string; tenantId: string }) {
  ...
  initiateTransfer(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() body: { fromUserId: string; toUserId: string; initiatedBy: string; tenantId: string },
  ) {
    return this.orgService.initiateOwnershipTransfer({ organizationId: id, ...body });
  }
...
export class SecurityAuditController {
  ...
  getAuditLogs(
    ...
    return this.auditService.getAuditLogs({ userId, tenantId, decision, page: +page, limit: +limit });
  }
