...
export class OrganizationService {
  ...
  async findById(id: string, tenantId: string) {
    ...
    if (!org) throw new NotFoundException(`Organization ${id} not found in tenant ${tenantId}`);
    return org;
  }
  ...
  async update(id: string, tenantId: string, data: Partial<{ name: string; logoUrl: string; address: string; city: string; status: string }>) {
  ...
  async addMember(organizationId: string, userId: string, role: string, tenantId: string) {
  ...
  async createInvitation(data: {
    ...
    const existing = await this.prisma.organizationInvitation.findFirst({
      where: { organizationId: data.organizationId, email: data.email, status: 'PENDING' },
    });
  ...
  async confirmOwnershipTransfer(plainToken: string, confirmingUserId: string) {
    ...
    if (!transfer) throw new NotFoundException('Invalid or expired transfer token');
    if (transfer.expiresAt < new Date()) throw new BadRequestException('Transfer token has expired');
    if (transfer.toUserId !== confirmingUserId) throw new BadRequestException('You are not the designated new owner');
    ...
    return { organizationId: transfer.organizationId, newOwnerId: transfer.toUserId };
  }
}
