...
export class TeacherAdminController {
  ...
  searchTeachers(
    ...
    return this.adminService.searchTeachers(
      search,
      parseInt(page),
      parseInt(limit),
    );
  ...
  @Get("metrics")
  @ApiOperation({
    summary: "[Admin] Get dashboard metrics (totals, pending, fraud flags)",
  })
  getDashboardMetrics() {
  ...
  @Put(":userId/feature")
  @ApiOperation({
    summary: "[Admin] Feature or un-feature a teacher on homepage",
  })
  featureTeacher(
  ...
  @ApiOperation({
    summary: "[Admin] Trigger stat recalculation for multiple teachers",
  })
