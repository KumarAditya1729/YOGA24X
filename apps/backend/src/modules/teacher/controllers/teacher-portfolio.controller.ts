...
export class TeacherPortfolioController {
  ...
  @ApiQuery({ name: 'type', required: false, description: 'Filter by PortfolioItemType' })
  ...
  @ApiOperation({ summary: 'Add a portfolio item (gallery image, video, award, etc.)' })
  ...
  updateItem(@Req() req: any, @Param('id') id: string, @Body() dto: Partial<AddPortfolioItemDto>) {
  ...
  publicPortfolio(@Param('userId') userId: string, @Query('type') type?: string) {
