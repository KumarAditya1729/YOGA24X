...
export class TeacherProfileController {
  ...
  @Get('me/profile')
  @ApiOperation({ summary: 'Get own teacher profile (full, with draft fields)' })
  ...
  removeSpecialization(@Req() req: any, @Param('specialization') specialization: string) {
    return this.profileService.removeSpecialization(req.user.userId, specialization);
  }

  // ── Teaching Preferences ─────────────────────────────────────────────────────

  @Put('me/teaching-preferences')
  @ApiOperation({ summary: 'Update teaching mode/format/age/skill preferences' })
  ...
  pinContent(
    ...
    return this.profileService.pinFeaturedContent(req.user.userId, contentType, contentId, parseInt(order) || 0);
  }
