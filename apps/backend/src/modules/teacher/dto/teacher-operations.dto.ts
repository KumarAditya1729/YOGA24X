import {
  ...
  MaxLength, Min, Max, IsDateString, IsUUID, IsNumber,
...
export class UpdateTeacherBookingRuleDto {
  @ApiPropertyOptional() @IsInt() @Min(0) @IsOptional() advanceBookingDays?: number;
  @ApiPropertyOptional() @IsInt() @Min(0) @IsOptional() minimumNoticeHours?: number;
  @ApiPropertyOptional() @IsInt() @Min(0) @IsOptional() cancellationWindowHours?: number;
  @ApiPropertyOptional() @IsInt() @Min(0) @IsOptional() reschedulePolicyHours?: number;
  @ApiPropertyOptional() @IsInt() @Min(0) @IsOptional() bufferTimeMinutes?: number;
  @ApiPropertyOptional() @IsInt() @Min(1) @IsOptional() maxDailySessions?: number;
  @ApiPropertyOptional() @IsInt() @Min(1) @IsOptional() maxWeeklyHours?: number;
  @ApiPropertyOptional() @IsInt() @Min(0) @IsOptional() lateArrivalGraceMins?: number;
  @ApiPropertyOptional() @IsBoolean() @IsOptional() preventOverbooking?: boolean;
...
export class CreateTeacherHolidayDto {
  ...
  @ApiPropertyOptional() @IsString() @IsOptional() @MaxLength(255) reason?: string;
...
export class CreateTeacherSessionTypeDto {
  ...
  @ApiProperty({ enum: SessionFormat }) @IsEnum(SessionFormat) format!: SessionFormat;
  @ApiProperty({ example: 60 }) @IsInt() @Min(15) durationMinutes!: number;
  @ApiProperty({ example: 5000 }) @IsInt() @Min(0) basePriceCents!: number;
  @ApiPropertyOptional({ example: 'USD' }) @IsString() @MaxLength(3) @IsOptional() currency?: string;
  @ApiPropertyOptional({ example: 1 }) @IsInt() @Min(1) @IsOptional() maxParticipants?: number;
...
export class UpdateTeacherSessionTypeDto {
  @ApiPropertyOptional() @IsString() @MaxLength(255) @IsOptional() title?: string;
  @ApiPropertyOptional() @IsString() @IsOptional() description?: string;
  @ApiPropertyOptional() @IsInt() @Min(15) @IsOptional() durationMinutes?: number;
...
export class CreateTeacherPricingRuleDto {
  ...
  @IsArray() @IsEnum(SessionFormat, { each: true }) @IsOptional() applicableFormats?: SessionFormat[];
}
...
export class CreatePayoutRequestDto {
  ...
  @ApiPropertyOptional({ example: 'USD' }) @IsString() @MaxLength(3) @IsOptional() currency?: string;
  @ApiPropertyOptional() @IsString() @MaxLength(100) @IsOptional() bankAccountId?: string;
}
