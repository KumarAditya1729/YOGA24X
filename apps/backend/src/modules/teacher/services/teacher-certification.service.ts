...
export class TeacherCertificationService {
  ...
  async addCertification(userId: string, dto: AddCertificationDto) {
    ...
    this.events.emit(TeacherEventType.CERT_ADDED, { userId, certificationId: cert.id });
    return cert;
  }
  ...
  async removeCertification(id: string, userId: string) {
    ...
    this.events.emit(TeacherEventType.CERT_REMOVED, { userId, certificationId: id });
    return { success: true };
  }
  ...
  async adminVerifyCertification(id: string, adminId: string, notes?: string) {
    ...
    this.events.emit(TeacherEventType.CERT_VERIFIED, { certificationId: id, verifiedBy: adminId });
    return updated;
  }

  async adminUnverifyCertification(id: string, adminId: string, notes?: string) {
    const cert = await this.certRepo.findById(id);
    if (!cert) throw new NotFoundException('Certification not found');
  ...
  private async assertOwnership(certId: string, userId: string) {
    ...
    if (!cert) throw new NotFoundException('Certification not found');
    if (cert.userId !== userId) throw new ForbiddenException('You do not own this certification');
  }
}
