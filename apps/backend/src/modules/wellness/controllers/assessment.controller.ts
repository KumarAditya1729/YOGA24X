...
export class AssessmentController {
  ...
  @ApiOperation({ summary: 'Submit new wellness assessment (stress, sleep, energy, BMI)' })
  ...
  async submitWellnessAssessment(@Request() req: any, @Body() body: any) {
    ...
    return this.assessService.submitWellnessAssessment(req.user.id, validated as SubmitWellnessAssessmentDto);
  }
  ...
  async getAssessmentHistory(@Request() req: any, @Query('limit') limit?: string) {
    return this.assessService.getAssessmentHistory(req.user.id, limit ? parseInt(limit, 10) : 30);
  }
  ...
  @Put('yoga')
  @ApiOperation({ summary: 'Update yoga assessment, experience, and preferred styles' })
  async updateYogaAssessment(@Request() req: any, @Body() body: any) {
    const validated = UpdateYogaAssessmentSchema.parse(body);
    return this.assessService.updateYogaAssessment(req.user.id, validated as UpdateYogaAssessmentDto);
  }
  ...
  @Put('nutrition')
  @ApiOperation({ summary: 'Update nutrition profile, calories goal, and meal timing' })
  async updateNutritionProfile(@Request() req: any, @Body() body: any) {
    const validated = UpdateNutritionProfileSchema.parse(body);
    return this.assessService.updateNutritionProfile(req.user.id, validated as UpdateNutritionProfileDto);
  }

  // 4. Meditation Profile
  @Get('meditation')
  @ApiOperation({ summary: 'Get user meditation profile and mindfulness goals' })
  async getMeditationProfile(@Request() req: any) {
    return this.assessService.getMeditationProfile(req.user.id);
  }

  @Put('meditation')
  @ApiOperation({ summary: 'Update meditation profile, preferred duration, and voice' })
  async updateMeditationProfile(@Request() req: any, @Body() body: any) {
    const validated = UpdateMeditationProfileSchema.parse(body);
    return this.assessService.updateMeditationProfile(req.user.id, validated as UpdateMeditationProfileDto);
  }

  // 5. AI Personalization Profile
  @Get('ai-personalization')
  @ApiOperation({ summary: 'Get AI coaching style, tone, and reminder behavior' })
  ...
  @ApiOperation({ summary: 'Update AI coaching style, motivation tone, and difficulty progression' })
  async updateAiPersonalization(@Request() req: any, @Body() body: any) {
    const validated = UpdateAiPersonalizationSchema.parse(body);
    return this.assessService.updateAiPersonalization(req.user.id, validated as UpdateAiPersonalizationDto);
  }
}
