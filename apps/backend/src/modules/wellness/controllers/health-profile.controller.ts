...
export class HealthProfileController {
  ...
  @Get()
  @ApiOperation({
    summary: "Get current user health profile and medical history",
  })
  @ApiResponse({
    status: 200,
  ...
  @Put()
  @ApiOperation({
    summary: "Update health profile, conditions, surgeries, and limitations",
  })
  @ApiResponse({
    status: 200,
    description: "Health profile updated and safety flags evaluated.",
  })
  async updateMyHealthProfile(@Request() req: any, @Body() body: any) {
    const validated = UpdateHealthProfileSchema.parse(body);
    return this.healthService.updateHealthProfile(
      req.user.id,
      validated as UpdateHealthProfileDto,
    );
  }
  ...
  @ApiOperation({
    summary: "Doctor verification of user health profile (Doctor/Admin only)",
  })
  @ApiResponse({
    status: 200,
    description: "Health profile verified by doctor.",
  })
  async verifyHealthProfile(@Request() req: any, @Body() body: any) {
    const validated = DoctorVerifyHealthProfileSchema.parse(body);
    return this.healthService.verifyHealthProfileByDoctor(
      req.user.id,
      validated as DoctorVerifyHealthProfileDto,
    );
  }
}
