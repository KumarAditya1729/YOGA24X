...
export class MedicalSafetyController {
  ...
  @Get("active")
  @ApiOperation({
    summary: "Get active medical safety flags and contraindications",
  })
  ...
  async createSafetyFlag(@Request() req: any, @Body() body: any) {
    const validated = CreateMedicalSafetyFlagSchema.parse({
      ...body,
      userId: req.user.id,
    });
    return this.safetyService.createSafetyFlag(
      validated as CreateMedicalSafetyFlagDto,
    );
  }
  ...
  @ApiOperation({
    summary: "Check if specific yoga poses are safe for the current user",
  })
  @ApiResponse({
    status: 200,
    description: "Returns safety check results and triggered warnings.",
  })
  async checkPoseRestrictions(
    @Request() req: any,
    @Body() body: { poseNames: string[] },
  ) {
    return this.safetyService.checkPoseRestrictions(
      req.user.id,
      body.poseNames || [],
    );
  }
