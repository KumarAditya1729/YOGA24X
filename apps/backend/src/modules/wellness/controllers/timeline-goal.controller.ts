...
export class TimelineGoalController {
  ...
  @ApiOperation({ summary: 'Log daily wellness timeline (mood, sleep, water, yoga minutes)' })
  @ApiResponse({ status: 200, description: 'Timeline logged and streak updated.' })
  async logDailyTimeline(@Request() req: any, @Body() body: any) {
    const validated = LogTimelineSchema.parse(body);
    return this.timelineGoalService.logDailyTimeline(req.user.id, validated as LogTimelineDto);
  }
  ...
  async getTimelineLogs(
    ...
    return this.timelineGoalService.getTimelineLogs(req.user.id, startDate, endDate);
  }

  // 2. User Goals
  @Post('goals')
  @HttpCode(HttpStatus.CREATED)
  @ApiOperation({ summary: 'Create new wellness goal (weight, flexibility, streak, custom)' })
  async createGoal(@Request() req: any, @Body() body: any) {
    const validated = CreateUserGoalSchema.parse(body);
    return this.timelineGoalService.createGoal(req.user.id, validated as CreateUserGoalDto);
  }
  ...
  async updateGoal(@Request() req: any, @Param('id') id: string, @Body() body: any) {
    ...
    return this.timelineGoalService.updateGoal(id, req.user.id, validated as UpdateUserGoalDto);
  }
