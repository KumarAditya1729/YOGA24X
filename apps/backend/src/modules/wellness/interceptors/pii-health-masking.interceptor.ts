...
export class PiiHealthMaskingInterceptor implements NestInterceptor {
  ...
  private maskHealthData(data: any, currentUser: any): any {
    ...
    const isOwner =
      currentUser &&
      (currentUser.id === masked.userId || currentUser.sub === masked.userId);
    ...
    if (!isOwner && !isDoctorOrAdmin) {
      if (masked.emergencyContactPhone) {
        masked.emergencyContactPhone = masked.emergencyContactPhone.replace(
          /.(?=.{4})/g,
          "*",
        );
      }
      if (
        masked.medicalHistoryJson &&
        Array.isArray(masked.medicalHistoryJson)
      ) {
        masked.medicalHistoryJson = masked.medicalHistoryJson.map(
          (item: any) => ({
            ...item,
