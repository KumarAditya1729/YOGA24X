...
export const UpdateBasicProfileSchema = z.object({
  ...
  phoneNumber: z.string().regex(/^\+[1-9]\d{6,14}$/, 'Invalid international phone format').optional(),
});
...
export const UpdateStudentProfileSchema = z.object({
  ...
  ayurvedicDosha: z.enum(['VATA', 'PITTA', 'KAPHA', 'VATA_PITTA', 'PITTA_KAPHA', 'VATA_KAPHA', 'TRIDOSHA']).optional(),
...
export const UpdateTeacherProfileSchema = z.object({
  ...
  specializations: z.array(z.string()).min(1, 'At least one specialization required').optional(),
...
export type UpdateNotificationPrefDto = z.infer<typeof UpdateNotificationPrefSchema>;
...
export const SubmitVerificationSchema = z.object({
  verificationType: z.enum(['EMAIL', 'PHONE', 'GOVERNMENT_ID', 'TEACHER_CERT', 'DOCTOR_REG', 'STUDIO_REG', 'CORPORATE_REG']),
...
export const UpdateConsentSchema = z.object({
  consentType: z.enum(['TERMS_OF_SERVICE', 'PRIVACY_POLICY', 'COOKIE_PREFERENCES', 'MARKETING_COMMUNICATIONS', 'AI_COACHING', 'HEALTH_DATA_PROCESSING']),
...
export const AccountDeletionRequestSchema = z.object({
  ...
  passwordConfirmation: z.string().min(1, 'Password confirmation required to request account deletion'),
});
